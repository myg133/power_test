...
use crate::storage::{effective_group_key, run_dir};
...
pub async fn run_with_cancel(opts: RunOptions, cancel: Arc<Notify>) -> Result<RunOutput> {
    ...
    // M6g: history is grouped by alias (if set) so dated
    // snapshots of the same underlying model land in the same
    // subdirectory. `effective_group_key` picks the alias when
    // present, otherwise the model name. `run_dir` sanitizes
    // the group key for filesystem use; the original strings
    // live in the saved config.json for accurate reporting.
    let group_key = effective_group_key(&cfg.model, cfg.model_alias.as_deref());
    let history_dir = run_dir(&opts.history_dir, group_key, &cfg.run_id);
