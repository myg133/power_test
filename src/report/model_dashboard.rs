...
fn build_runs_table(
    ...
    for r in runs {
        ...
        let _ = write!(
            out,
            // The first cell wraps the run id in an <a> so
            // middle-click / right-click / copy-link work
            // natively without going through the row click
            // handler. The link points at the same report.html
            // the row click would open, and the row click
            // handler explicitly bails when the click came
            // from inside an <a> (see the script below).
            "<td><a class=\"run-link\" href=\"{report}\" target=\"_blank\" rel=\"noopener noreferrer\"><code>{run_id}</code></a></td>\
            <td class=\"target\" title=\"{target_title}\"><a class=\"url\" href=\"{target_url}\" target=\"_blank\" rel=\"noopener noreferrer\">{target_text}</a></td>\
            <td class=\"num\">{:.2}</td>\
            <td class=\"num\">{}s</td>\
            <td class=\"num\">{}</td>\
            <td class=\"num\">{:.1}%</td>\
            ...
            report = html_escape(&r.report_filename),
            run_id = html_escape(&r.run_id),
            target_title = html_escape(&r.target),
            target_url = html_escape(&r.target),
            target_text = html_escape(&r.target),
        );
        out.push_str("</tr>");
    }
...
"##.to_string();
    ...
    body.push_str("  <script>\n    // M7 fix: make the runs table rows clickable so
    // clicking anywhere in the row opens the run's
    // report.html in a new tab. The run_id cell wraps
    // the id in <a class=\"run-link\"> with the same href,
    // so middle-click / right-click / copy-link work
    // natively. The row click bails when the click
    // originated inside an <a>, so we never double-open.
    (function() {
      document.querySelectorAll('tr.row').forEach(function(row) {
        row.addEventListener('click', function(ev) {
          // If the click was on the run-id link or the
          // target URL, the browser handles navigation.
          if (ev.target.closest('a')) { return; }
          var href = row.getAttribute('data-report');
          if (!href) { return; }
...
mod tests {
    use super::*;
    use crate::config::{ApiKind, DatasetSpec, LoadPattern, PromptDistribution, RequestStrategy, RunConfig, PromptSource};
    ...
    fn dummy_run(id: &str) -> RunSummary {
        let agg = crate::runner::MetricsAggregator::new();
        run_summary_from_run(&cfg(), &agg, format!("{id}/report.html"))
    }

    /// M7 fix: the run_id cell must be wrapped in an `<a
    /// class="run-link">` whose href points at the
    /// run's `report.html`. Without this, the run id
    /// was a plain `<code>` and the user had to click
    /// somewhere ambiguous to open the report.
    #[test]
    fn run_id_in_dashboard_is_clickable_link_to_report() {
        let runs = vec![dummy_run("aabbcc-1234")];
        let html = render_dashboard("gpt-3.5-turbo", "gpt-3.5-turbo", None, &runs);
        // The run id must be wrapped in a run-link that
        // points at the report.
        assert!(
            html.contains(r#"<a class="run-link" href="aabbcc-1234/report.html""#),
            "expected run-link to the report; rendered html: {}",
            html
        );
        // The whole row must carry the same data-report
        // attribute (used by the JS row click handler).
        assert!(
            html.contains(r#"data-report="aabbcc-1234/report.html""#),
            "expected tr.row data-report attribute"
        );
    }

    /// M7 fix: clicking the run id (which is now an
    /// anchor) should NOT also trigger the row click
    /// handler — the JS explicitly bails when the click
    /// originated inside an <a>. We assert by checking
    /// the JS contains the bail-out branch.
    #[test]
    fn row_click_handler_bails_when_click_inside_anchor() {
        let html = render_dashboard("gpt-3.5-turbo", "gpt-3.5-turbo", None, &[]);
        // The row click script is present.
        assert!(
            html.contains("querySelectorAll('tr.row')"),
            "row click handler must be wired up"
        );
        // And it explicitly checks the click target is
        // not inside an anchor, so middle-click /
        // right-click on the run-link works natively.
        assert!(
            html.contains("ev.target.closest('a')"),
            "row click must bail when click is on an anchor"
        );
    }

    /// M7 i18n: the advanced-metrics card label changed
    /// from "M8 指标" to "高级指标" and the per-metric
    /// labels were translated to Chinese. This test
    /// asserts the renamed + translated values land in
    /// the i18n dict.
    #[test]
    fn advanced_metrics_card_is_translated_to_chinese() {
        // The default render of the report uses Locale::Zh
        // and the i18n dict drives every data-i18n element.
        let agg = crate::runner::MetricsAggregator::new();
        let html = render_html(&cfg(), &agg, false);
        // The card heading is now "高级指标" (Advanced metrics)
        // and the row labels are translated. We assert the
        // Chinese strings appear in the page body (not
        // inside the i18n-dict-en block which also contains
        // English copies).
        assert!(
            html.contains("高级指标"),
            "advanced-metrics card heading must be '高级指标'"
        );
        assert!(
            html.contains("TPOT (毫秒/token)"),
            "TPOT row label must be translated"
        );
        assert!(
            html.contains("平均输入 token"),
            "avg input row label must be translated"
        );
        assert!(
            html.contains("平均输出 token"),
            "avg output row label must be translated"
        );
    }
}
